...
const FluidCursor = () => {
  ...
  useEffect(() => {
    ...
    // Material class for shader programs with keywords
    class Material {
    ...
    const generateColor = () => {
      let c;
      if (isDark) {
        // Dark theme: golden/amber rainbow
        const hue = Math.random() * 0.15 + 0.08; // 0.08-0.23 (orange to yellow range)
        c = HSVtoRGB(hue, 0.8, 1.0);
      } else {
        // Light theme: LinkedIn blue variations
        const hue = Math.random() * 0.1 + 0.55; // 0.55-0.65 (blue range)
        c = HSVtoRGB(hue, 0.7, 1.0);
      }
      c.r *= 0.15;
      c.g *= 0.15;
      c.b *= 0.15;
    ...
    // First interaction initializes the loop
    ...
    const handleFirstInteraction = (e) => {
      if (!initialized) {
        ...
        if (e.touches) {
          posX = scaleByPixelRatio(e.touches[0].clientX);
          posY = scaleByPixelRatio(e.touches[0].clientY);
        } else {
          posX = scaleByPixelRatio(e.clientX - rect.left);
          posY = scaleByPixelRatio(e.clientY - rect.top);
        }
        ...
        document.body.removeEventListener('mousemove', handleFirstInteraction);
        document.body.removeEventListener('touchstart', handleFirstInteraction);
      }
    };

    // Attach event listeners
    document.body.addEventListener('mousemove', handleFirstInteraction);
